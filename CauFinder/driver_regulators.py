...
def MDScontrol(directed_graph: nx.DiGraph, nodes_importance: pd.Series, solver='GUROBI', lam=0.0):
    ...
    if reduced_graph.number_of_nodes() == 0:
        ...
        score_terms = sum([nodes_importance[node] * x[i] for i, node in enumerate(reduced_graph.nodes())])
        # print("score_terms:", score_terms)
        obj = cvx.Minimize(cvx.sum(x) - lam * score_terms)
...
def _CORE(directed_graph: nx.DiGraph, nodes_importance: pd.Series, S: set):
    remove_set = set()
...
def _MFVS_graph_reduction(directed_graph: nx.DiGraph, nodes_importance: pd.Series, S: set,
                          execute_in0out0=True, execute_selfloop=True, execute_out1=False,
                          execute_in1=False, execute_PIE=True, execute_CORE=False, execute_DOME=False):
    all_done = False
    while not all_done:
        all_done = True
...
def MFVScontrol(directed_graph: nx.DiGraph, nodes_importance: pd.Series, solver='GUROBI', lam=0.0, **kwargs):
    ...
    while True:
        # Graph reduction
        reduced_graph, critical_nodes = _MFVS_graph_reduction(reduced_graph, nodes_importance, critical_nodes, **kwargs)
        ...
        if len(reduced_graph_SCCs) > 1:
            ...
            for g in reduced_graph_SCCs:
                if len(g) > 1:
                    ...
                    g, critical_nodes = _MFVS_graph_reduction(g, nodes_importance, critical_nodes, **kwargs)
                    # reduced_graph_new.update(g)
                    reduced_graph_new.add_nodes_from(g.nodes(data=True))
                    reduced_graph_new.add_edges_from(g.edges(data=True))
        ...
        if reduced_graph.number_of_nodes() > 500:
    ...
    if reduced_graph.number_of_nodes() == 0:
        ...
        # Define the optimization problem
        # obj = cvx.Minimize(cvx.sum(x))
        score_terms = sum([nodes_importance[node] * x[nodes_idx_map[node]] for node in reduced_graph.nodes()])
        # print("score_terms:", score_terms)
        obj = cvx.Minimize(cvx.sum(x) - lam * score_terms)
        # Solve the problem
        prob = cvx.Problem(obj, constraints)
        ...
        # Set remain nodes that belongs to the FVS as critical nodes
...
def highly_weighted_genes(gene_influence_scores: pd.DataFrame, topK: int = 50):
    ...
    v_all = gene_influence_scores.sort_values(by='score', ascending=False)
    v_all = v_all.iloc[0:topK, [2]]
    critical_genes = set(v_all[v_all > 0].index)
...
def driver_regulators(GRN_nx: nx.DiGraph,
    ...
    # MFVS driver nodes
    node_score = gene_influence_score.loc[:, 'score']
    MFVS_driver_set, source_nodes = MFVScontrol(GRN_nx, node_score, solver=solver, lam=lam, **kwargs)
    MFVS_driver_set = MFVS_driver_set.union(source_nodes)
    # # MDS driver nodes
    # MDS_driver_set, MDS_intermittent_nodes = MDScontrol(GRN_nx, node_score, solver=solver, lam=lam)
    ...
    # Top-ranked genes based on the gene causal influence score
    # critical_genes = highly_weighted_genes(gene_influence_score, topK)
    critical_genes, out_critical_genes, in_critical_genes = highly_weighted_genes(gene_influence_score, topK)
    print('  {} top-rank genes are found.'.format(len(critical_genes)))

    # The final driver regulators identified by CauFinder
    # causal_drivers = driver_set.intersection(critical_genes)
    CauFVS_driver_set = critical_genes.intersection(MFVS_driver_set)
    print("The number of causal FVS driver:", len(CauFVS_driver_set))

    max_attempts = 5  # set maximum attempts to 5
    attempts = 0  # initialize attempts counter
    while len(CauFVS_driver_set) < 20:  # for mESC dataset, set to 20 for benchmarking
        # for debug purpose
        print("The number of CauFVS drivers is less than 15, try to increase the topK value.")
        critical_genes, out_critical_genes, in_critical_genes = highly_weighted_genes(gene_influence_score, topK+10)
        # causal_drivers = driver_set.intersection(critical_genes)
        CauFVS_driver_set = critical_genes.intersection(MFVS_driver_set)
        # print("The number of causal drivers:", len(causal_drivers))
        attempts += 1  # increment attempts counter

        if attempts >= max_attempts:
            print("Maximum attempts reached, stopping loop.")
            break  # break loop if maximum attempts reached
    ...
    # drivers_df['is_causal_driver'] = drivers_df.index.isin(list(causal_drivers))
    drivers_df['is_MFVS_driver'] = drivers_df.index.isin(list(MFVS_driver_set))
    drivers_df['is_CauFVS_driver'] = drivers_df.index.isin(list(CauFVS_driver_set))
    # drivers_df['is_MDS_driver'] = drivers_df.index.isin(list(MDS_driver_set))
    drivers_df['is_CauW_driver'] = drivers_df.index.isin(list(critical_genes))
    drivers_df.sort_values(by='score', ascending=False, inplace=True)
