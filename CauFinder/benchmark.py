from CauFinder.dataloader import generate_synthetic
import numpy as np
import pandas as pd
...
def run_caufinder121(
    ...
    grad_df = cumulative_weight_sum_rate(
        model.get_feature_weights(sort_by_weight=True, method="Grad")[0]
    )
    # both_df = cumulative_weight_sum_rate(
...
def run_caufinder_nc(
    ...
    model.train(
        max_epochs=300,
        rec_loss1_weight=1.0,
        rec_loss2_weight=1.0,
        z_kl_weight=0.1,
        feat_l1_weight=0.01,
        dpd_weight=2.0,
        fide_kl_weight=0.0,
            ...
            if G[x][y]:
                ...
                if len(neighbors) >= l:  # |adj(C, x) \ {y}| > l
                    ...
                    for K in set(combinations(neighbors, l)):
                        # Check if nodes x and y are d-separated by subset K
                        # Perform conditional independence test, return p-value
                        p_value = gauss_ci_test(suff_stat, x, y, list(K))
                        if p_value > p_value_mat[x][y]:
                            p_value_mat[x][y] = p_value_mat[y][x] = p_value
                        # Conditional independence
                        if p_value >= alpha:
                            G[x][y] = G[y][x] = False  # Remove edge x -- y
                            O[x][y] = O[y][x] = list(K)  # Add K to separating sets O
                            break
...
def extend_cpdag(G, O):
    ...
    def rule1(g):
        """Rule 1: If there exists a chain i -> j - k, and i and k are not adjacent, then it becomes i -> j -> k"""
        pairs = [
            (i, j)
    ...
    def rule2(g):
        """Rule 2: If there exists a chain i -> k -> j, then change i - j to i -> j"""
        pairs = [
            (i, j)
            for i in range(n_nodes)
            for j in range(n_nodes)
    ...
    def rule3(g):
        """Rule 3: If there exist i - k1 -> j and i - k2 -> j, and k1 and k2 are not adjacent, then change i - j to i -> j"""
        pairs = [
            (i, j)
            for i in range(n_nodes)
            for j in range(n_nodes)
...
def run_pc(X, y, var_df, sort_by_weight=True):
    ...
    # Get the p-values corresponding to the last column
    pv = pvm[:-1, -1]

    # Update var_df
    var_df["weight"] = 1 - pv
    # var_df["pred_label"] = (pv < alpha).astype(int)  # Set prediction label based on p-values less than alpha
...
def run_benchmark(
    ...
    **model_kwargs,
    ...
    for i in range(n_dataset):
        ...
        # # MLP_SHAP&Grad result
        # mlp_shap_res, mlp_grad_res = run_mlp(X, y, var_df.copy(), n_hidden=64, n_layers=2, sort_by_weight=True)
        # # mlp_res = mlp_res.merge(label, on='features', how='left')
        # mlp_shap_result.append(mlp_shap_res)
        # mlp_grad_result.append(mlp_grad_res)
        #
    ...
    create_radar_chart(score_dict, full_folder_path)

    return score_dict, result_dict


def create_radar_chart(score_dict, folder_path):
    # obtain the methods and categories
    methods = score_dict["AUC"].index.tolist()
    # define the number of categories
    categories = [key for key in score_dict.keys() if key != "num_w_cum_rate"]
    # categories = ['AUC', 'ACC', 'MCC', 'Precision', 'Specificity', 'Recall', 'F1_score']
    ...
    for i, method in enumerate(methods):
        ...
        for category in categories:
            mean_value = score_dict[category].loc[method, 'Mean']
            values.append(mean_value)
        values += values[:1]

        ax.plot(angles, values, color=colors(i), linewidth=2, linestyle='solid', label=method)
    ...
    ax.legend(loc='center left', bbox_to_anchor=(1.1, 0.5), fontsize=12, frameon=False)

    # Add title
    plt.title("Comparison of Methods across Different Metrics", size=16, fontweight='bold')

    # Save the chart as PDF and PNG
    plt.savefig(os.path.join(folder_path, "radar_chart.pdf"), format="pdf", bbox_inches='tight')
    plt.savefig(os.path.join(folder_path, "radar_chart.png"), format="png", bbox_inches='tight')
    plt.close()
...
def run_ablation(
    ...
    **model_kwargs,
    ...
    for i in range(n_dataset):
        ...
        # Caufinder NC result
        cf_nc_shap_res, cf_nc_grad_res = run_caufinder_nc(
            adata=adata,
            **model_kwargs
        )
        caufinder_nc_grad_result.append(cf_nc_grad_res)
    ...
    result_dict = {
        ...
        # "CAUF121": caufinder121_grad_result,
        "CauFinder": caufinder121_grad_result,
        "CauFinderNC": caufinder_nc_grad_result,
        # "PC": pc_result,
        # "VAE_SHAP": vae_shap_result,
        # "VAE_Grad": vae_grad_result,
        # "RF": rf_result,
...
def generate_boxplot_csv(
    ...
    sns.swarmplot(data=df, color='k', alpha=0.7, size=6)

    plt.xticks(rotation=45, ha="right")
    plt.xlabel("Methods")
    plt.ylabel(file_start_name)
